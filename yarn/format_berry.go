...
func (f *YarnBerryV8Formatter) FormatFromResult(result *ResolveResult, project *ecosystem.ProjectSpec) ([]byte, error) {
	return formatBerry(result, project, 8, 10)
}
...
func writeEntryBody(b *strings.Builder, pkg *ResolvedPackage) {
	node := pkg.Node
	...
	if node.Integrity != "" {
		checksum := integrityToYarnChecksum(node.Integrity)
		if checksum != "" {
...
func integrityToYarnChecksum(integrity string) string {
	...
	return "10/" + hex.EncodeToString(decoded)
}
